...
export const format_asset = (asset_hash: string, atomic_amount: number, with_ticker: boolean, locale?: Intl.LocalesArgument) => {
    const asset = assets[asset_hash];
    if (asset) {
        const amount = parse_atomic(atomic_amount, asset.decimals);
        let value = `${amount.toLocaleString(locale, { maximumFractionDigits: asset.decimals })}`;
