        ...
        const load = async () => {
            for (let i = 0; i < contract_logs.length; i++) {
                ...
                if (contract_log.type === `exit_code`) {
                    ...
                    if (exit_code === 0) {
                        collapse_box.content_element.innerHTML = `<div class="exit-code"><span class="dot-success"></span>The contract execution was successful. Exit code: 0</div>`;
                    } else {
                        collapse_box.content_element.innerHTML = `<div class="exit-code"><span class="dot-fail"></span>The contract execution failed. Exit code: ${exit_code}</div>`;
                    }
