...
export class TransactionInfo {
    ...
    set_fees(tx: TransactionResponse) {
        this.fees_element.innerHTML = `
            <div>FEES</div>
            <div>
                <div><div>MIN</div><div>${format_xel(tx.fee, true)}</div></div>
                <div><div>PAID</div><div>${tx.fee_paid ? format_xel(tx.fee_paid, true) : `--`}</div></div>
                <div><div>MAX</div><div>${format_xel(tx.fee_limit, true)}</div></div>
            </div>
        `;
    }
